...
class SosialController extends Controller
{
    ...
    {
        $ipds = Item::sosialItem();
        $workspace = Wokrspace::sosialWorkspace();
        $anggota = Anggota::all(['nama']);

        if(auth()->user()->seksi_id == 4 || auth()->user()->status == 'Admin') {
            return view('sosial', ['items' => $ipds, 'workspace' => $workspace, 'anggota' => $anggota]);
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect('/sosial')->with(['sukses' => 'Data Berhasil Disimpan']);
    }

    public function destroy($workspace){
        if(Item::where('workspace_id',$workspace)->exists()){
            return redirect('sosial')->with(['sukses' => 'Hapus semua task terlebih dahulu']);
        } else {
            Wokrspace::destroy($workspace);
            
            return redirect('sosial')->with(['sukses' => 'Data Berhasil Dihapus']);
        }
    }
    ...
    public function update(Request $request,$workspace): RedirectResponse
    {
        ...
        if($updated){
            return redirect('sosial')->with(['sukses' => 'Data Berhasil Diupdate']);
        } else {
            return redirect('sosial')->with(['sukses' => 'Data Gagal diedit']);
        }
    }
    ...
    public function updateItems(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('sosial')->with(['sukses' => 'Data Berhasil diedit']);
    }
    ...
    public function upload(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('sosial')->with(['sukses'=>'Data Berhasil '. $request->upload]);
    }
}

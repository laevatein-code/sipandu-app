...
class TuController extends Controller
{
    ...
    {
        $ipds = Item::tuItems();
        $workspace = Wokrspace::tuWorspace();
        $anggota = Anggota::all(['nama']);

        if(auth()->user()->seksi_id == 6 || auth()->user()->status == 'Admin') {
            return view('tata-usaha', ['items' => $ipds, 'workspace' => $workspace, 'anggota' => $anggota]);
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect('/tata-usaha')->with(['sukses' => 'Data Berhasil Disimpan']);
    }

    public function destroy($workspace){
        if(Item::where('workspace_id',$workspace)->exists()){
            return redirect('tata-usaha')->with(['sukses' => 'Hapus semua task terlebih dahulu']);
        } else {
            Wokrspace::destroy($workspace);
            
            return redirect('tata-usaha')->with(['sukses' => 'Data Berhasil Dihapus']);
        }
    }
    ...
    public function update(Request $request,$workspace): RedirectResponse
    {
        ...
        if($updated){
            return redirect('tata-usaha')->with(['sukses' => 'Data Berhasil Diupdate']);
        } else {
            return redirect('tata-usaha')->with(['sukses' => 'Data Gagal diedit']);
        }
    }
    ...
    public function updateItems(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('tata-usaha')->with(['sukses' => 'Data Berhasil diedit']);
    }
    ...
    public function upload(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('tata-usaha')->with(['sukses'=>'Data Berhasil '. $request->upload]);
    }
}

...
class ProduksiController extends Controller
{
    ...
    {
        $ipds = Item::produksiItem();
        $workspace = Wokrspace::produksiWorkspace();
        $anggota = Anggota::all(['nama']);

        if(auth()->user()->seksi_id == 3 || auth()->user()->status == 'Admin') {
            return view('produksi', ['items' => $ipds, 'workspace' => $workspace, 'anggota' => $anggota]);
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect('/produksi')->with(['sukses' => 'Data Berhasil Disimpan']);
    }

    public function destroy($workspace){
        if(Item::where('workspace_id',$workspace)->exists()){
            return redirect('produksi')->with(['sukses' => 'Hapus semua task terlebih dahulu']);
        } else {
            Wokrspace::destroy($workspace);
            
            return redirect('produksi')->with(['sukses' => 'Data Berhasil Dihapus']);
        }
    }
    ...
    public function update(Request $request,$workspace): RedirectResponse
    {
        ...
        if($updated){
            return redirect('produksi')->with(['sukses' => 'Data Berhasil Diupdate']);
        } else {
            return redirect('produksi')->with(['sukses' => 'Data Gagal diedit']);
        }
    }
    ...
    public function updateItems(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('produksi')->with(['sukses' => 'Data Berhasil diedit']);
    }
    ...
    public function upload(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('produksi')->with(['sukses'=>'Data Berhasil '. $request->upload]);
    }
}

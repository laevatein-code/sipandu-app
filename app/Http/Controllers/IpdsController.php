...
class IpdsController extends Controller
{
    public function index() : View
    {
        ...
        if(auth()->user()->seksi_id == 1 || auth()->user()->status == 'Admin') {
            return view('ipds', ['items' => $ipds, 'workspace' => $workspace, 'anggota' => $anggota]);
        }
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect('/ipds')->with(['sukses' => 'Data Berhasil Disimpan']);
    }

    public function destroy($workspace){
        if(Item::where('workspace_id',$workspace)->exists()){
            return redirect('ipds')->with(['sukses' => 'Hapus semua task terlebih dahulu']);
        } else {
            Wokrspace::destroy($workspace);
            
            return redirect('ipds')->with(['sukses' => 'Data Berhasil Dihapus']);
        }
    }
    ...
    public function update(Request $request,$workspace): RedirectResponse
    {
        ...
        if($updated){
            return redirect('ipds')->with(['sukses' => 'Data Berhasil Diupdate']);
        } else {
            return redirect('ipds')->with(['sukses' => 'Data Gagal diedit']);
        }
    }
    ...
    public function updateItems(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('ipds')->with(['sukses' => 'Data Berhasil diedit']);
    }
    ...
    public function upload(Request $request, $id): RedirectResponse
    {
        ...
        return redirect('ipds')->with(['sukses'=>'Data Berhasil '. $request->upload]);
    }
}
